...
export async function PUT(request: Request) {
  try {
    ...
    // Recalcular el total de la orden después de actualizar los items
    const totalResult = await sql`
      SELECT SUM(price * quantity) as total FROM order_items WHERE order_id = ${id}
    `
    const newTotal = totalResult[0]?.total || 0
    await sql`
      UPDATE orders SET total_amount = ${newTotal} WHERE id = ${id}
    `
    updatedOrder[0].total_amount = newTotal
