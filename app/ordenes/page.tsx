...
export default function OrdersPage() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "pending" | "completed" | "delivered" | "ready">("all")
  ...
  const filteredOrders = Array.isArray(orders)
    ? orders.filter((order) => {
        ...
        return (
          order.table_reference?.toString().toLowerCase().includes(term) ||
          order.table_id?.toString().toLowerCase().includes(term) ||
          order.customer_name?.toLowerCase().includes(term) ||
          order.details?.toLowerCase().includes(term) ||
          (order.status && (
            (order.status === "pending" && "pendiente".includes(term)) ||
            (order.status === "preparing" && "preparando".includes(term)) ||
            (order.status === "ready" && "listo".includes(term)) ||
            (order.status === "delivered" && "entregado".includes(term)) ||
            (order.status === "completed" && "completado".includes(term)) ||
      ...
      <main className="main-content">
        <div className="container">
          <div className="card">
            ...
            <div className="tabs">
              <div className={`tab ${activeTab === "all" ? "active" : ""}`} onClick={() => setActiveTab("all")}>Todas</div>
              <div className={`tab ${activeTab === "pending" ? "active" : ""}`} onClick={() => setActiveTab("pending")}>Pendientes</div>
              <div className={`tab ${activeTab === "ready" ? "active" : ""}`} onClick={() => setActiveTab("ready")}>Listas</div>
              <div className={`tab ${activeTab === "delivered" ? "active" : ""}`} onClick={() => setActiveTab("delivered")}>Entregadas</div>
              <div className={`tab ${activeTab === "completed" ? "active" : ""}`} onClick={() => setActiveTab("completed")}>Completadas</div>
            </div>
