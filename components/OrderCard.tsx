...
export default function OrderCard({ order, onClick, onUpdateStatus }: OrderCardProps) {
  ...
  if (order.status === "pending") {
    ...
    contentClass = "order-content-ready";
  } else if (order.status === "preparing" || order.status === "delivered") {
    cardClass += " order-card-preparing";
    contentClass = "order-content-preparing";
          ...
          <ul className="order-products-list">
            {order.items.map((item, index) => (
              <li key={index} className="order-product-item">
                {item.quantity}x {item.product?.name || item.product_name || `Producto #${item.product_id}`}
              </li>
            ))}
          </ul>
        </div>
